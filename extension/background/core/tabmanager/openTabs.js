 ...
 */
async function openListOfTabs(tabsToOpen, windowId, {
  inLastPos = false,
  ...
  try {
    ...
    if (activeIndex >= 0) {
      ...
      createdTabs[activeIndex] = await openTab(activeTab, windowId, indexTabOffset);
    ...
    for (let tab of tabsToOpen) {
      ...
      if (index !== activeIndex) {
        let currentIndex = (tab.pinned)
          ...
          : indexTabOffset;
        // Save results
        createdTabs[index] = await openTab(tab, windowId, currentIndex, tabIdsCrossRef);
